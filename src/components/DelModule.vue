<template>
  ...
  <div class="row" v-else>
    <div class="page-title" style="width: 100%; margin-bottom: 15px; padding: 5px;">
      <h3 style="text-align: center; width: 100%;">Списание со склада</h3>
    </div>
    ...
    async submitModuleProd () {
      try {
        for (const item of this.items) {
          ...
          const addPositionProduct = await this.$store.dispatch('insertCompInProduct', { areaId: this.$route.params.areaId, id: item.id, howleft: howleft })
          this.$emit('phucked', addPositionProduct)
          this.$message('Успешно')
        }
        this.$modal.hide('del-extra')
      } catch (e) {}
    },
